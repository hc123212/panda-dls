...
*蓝色: 期望末端轨迹（抬升显示）· 金色: 实测末端轨迹（绘制于指尖）*
...
- 场景内末端轨迹叠加：viewer 与 GIF/MP4 中同时绘制期望轨迹（蓝）与实测末端轨迹（金），`trail.py` 向 `MjvScene` 用户 geom 通道写入小球，不改模型 XML；
...
| 空间圆跟踪 · 动力学层（PD 位置舵机，保留作基线） | 位置 RMS $4.2$ mm，姿态 max $1.28^\circ$ |
| 空间圆跟踪 · 力矩级操作空间控制（`osc.py` + `panda_motor.xml`） | 位置 RMS $0.002$ mm，姿态 max $0.0007^\circ$，力矩峰值 $23$ N·m |
...
- 误差与约束都来自优化：参考轨迹给出期望位姿与前馈，手写 MDH 运动学算出当前位姿与雅可比，DLS/QP 把 6D 误差映射为关节速度，MuJoCo 负责积分与渲染。执行层三选一：运动学层（直接积分）、PD 位置舵机（基线）、力矩级操作空间控制（`osc.py`，重力/科氏/惯量全前馈）。
...
本项目是个人学习性质的机械臂运动控制实现。执行层为三层架构（讲解文档 §9）：运动学层已对到实机频率精度；PD 位置舵机路径**保留作对照基线**；力矩级操作空间控制（Khatib，任务空间动力学全前馈）作为第三层新增。约束处理从"解析投影 + 事后裁剪"升级为 OSQP 硬约束 QP。后续方向：阻抗控制、任务优先级栈、TOPP-RA 时间参数化、C++/Eigen 移植与真机接口。
